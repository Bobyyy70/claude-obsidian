...
var Time = class {
  ...
  parse_time(st, date = this.today, nearest = true) {
    ...
    if (items) {
      let t = (0, import_obsidian22.moment)(`${date} ${items[1]}:${items[2]}:00`, "YYYY-MM-DD HH:mm:ss");
      if (t.isValid()) {
        return t;
      }
    }
    ...
    if (match) {
      ...
      if (["\u4E0B\u5348"].includes(period)) {
        ...
        let t = (0, import_obsidian22.moment)();
      ...
      hour %= 24;
      return (0, import_obsidian22.moment)(`${date} ${hour}:${minute}`, "YYYY-MM-DD HH:mm");
    }
    return null;
  }
...
`);
  ...
  async handleOnlineResolveNote(req, res, parsedUrl) {
    ...
    try {
      ...
      if (!tfile || !(tfile instanceof import_obsidian25.TFile) || tfile.extension !== "md") {
  ...
  async handleOnlineRender(req, res) {
    ...
    try {
      ...
      comp = new import_obsidian25.Component();
      comp.load();
      ...
      await import_obsidian25.MarkdownRenderer.render(this.app, markdown, el, sourcePath, comp);
  ...
  async handleOnlineResolveLink(req, res, parsedUrl) {
    try {
      ...
      if (!dest || !(dest instanceof import_obsidian25.TFile) || dest.extension !== "md") {
        res.writeHead(404, { "Content-Type": "application/json; charset=utf-8" });
...
var DailyJob = class {
  constructor(plugin) {
    ...
    this.date = this.tfile ? this.tfile.basename : (0, import_obsidian26.moment)().format("YYYY-MM-DD");
  }
  parse_fields(item) {
  ...
  async select_start_time(jobs, is_today = true) {
    ...
    if (is_today && timeList) {
      let ct = (0, import_obsidian26.moment)().format("HH:mm");
      timeList = timeList.filter((x) => x && x.localeCompare(ct) < 0);
    }
    if (timeList.length == 0) {
      new import_obsidian26.Notice("\u5DF2\u8BB0\u5F55\uFF01");
      return null;
    }
  ...
  async select_x_time(st = null) {
    ...
    if (stMoment) {
      let ct = (0, import_obsidian26.moment)().add(5, "minutes");
      ...
      if (this.plugin.easyapi.time.as_date(ct).isSame(this.plugin.easyapi.time.as_date(stMoment), "day")) {
        for (let i of [15, 10, 5, 0]) {
          let diffMinutes = ((0, import_obsidian26.moment)().valueOf() - stMoment.valueOf()) / 1e3 / 60;
...
${msg}`;
  ...
  async prase_job_item_v3(ctx, delta = 5) {
    ...
    if (items[2]) {
      ...
      let et = this.plugin.easyapi.time.parse_time((0, import_obsidian26.moment)().format("HH:mm"), this.date);
      if (!et) {
        return null;
      }
  ...
  async prase_job_item_v4() {
    ...
    jobs = jobs || [];
    let is_today = tfile.basename == (0, import_obsidian26.moment)().format("YYYY-MM-DD");
...
var BaseWebViewer = class {
  ...
  async request(ctx, timeout = 120) {
    ...
    if (N2 >= N1 + 1) {
      ...
      new import_obsidian27.Notice(`${this.name} \u8BF4\u4E86\u70B9\u4EC0\u4E48`);
      return ctx2;
    } else {
      new import_obsidian27.Notice(`${this.name} \u4E0D\u8BF4\u8BDD`);
...
${rsp}`;
  ...
  async cmd_chat_with_target_tfile(tfile = null, target = null) {
    ...
    let prompt = "";
    if (tfile instanceof import_obsidian28.TFile) {
...
${ctx}`;
    ...
    if (!hasSelection && prompt.includes("${selection}")) {
      new import_obsidian28.Notice("\u8BF7\u9009\u62E9\u6587\u672C/Select text first");
      return;
    }
    const sparasRegex = /\$\{\[\[(.*?)\]\]\}/g;
    ...
    if (tfile instanceof import_obsidian28.TFile && ea.editor.get_frontmatter(tfile, "reference", "link") != false) {
      let refFiles = [tfile];
      ...
      if (refFiles.length > 0) {
        const selectedLinks = await ea.dialog_multi_suggest(
          refFiles.map((x) => x instanceof import_obsidian28.TFile ? x.basename : x),
          refFiles,
          "",
    ...
    if (this.plugin.settings.webviewllm.write_clipboard == "1") {
      ...
      if (!copied) {
        new import_obsidian28.Notice(this.easyapi.isZh ? "\u590D\u5236\u5931\u8D25\uFF0C\u8BF7\u68C0\u67E5\u526A\u8D34\u677F\u6743\u9650" : "Copy failed, please check clipboard permission");
      } else {
        new import_obsidian28.Notice(this.easyapi.isZh ? "\u63D0\u793A\u8BCD\u5DF2\u590D\u5236" : "Prompt copied");
      }
    } else if (this.plugin.settings.webviewllm.write_clipboard == "2") {
      llm = await this.get_last_active_llm();
      if (!llm) {
        new import_obsidian28.Notice(this.easyapi.isZh ? "\u672A\u627E\u5230\u6D3B\u52A8\u7684 LLM Webview\uFF0C\u5DF2\u590D\u5236\u63D0\u793A\u8BCD" : "No active LLM webview found, prompt copied to clipboard");
      ...
      llm = await this.get_last_active_llm();
      if (!llm) {
        new import_obsidian28.Notice(this.easyapi.isZh ? "\u672A\u627E\u5230\u6D3B\u52A8\u7684 LLM Webview" : "No active LLM webview found");
        return;
      }
  ...
  async append_reference(refFiles, with_refPreamble = true) {
    ...
    for (const link of refFiles) {
      const body = link instanceof import_obsidian28.TFile ? await this.easyapi.nc.editor.remove_metadata(link) : await this.easyapi.fs.read_file(link);
      refBlocks.push(
        [
          D0,
          `name: ${link instanceof import_obsidian28.TFile ? link.basename : this.easyapi.fs.path.basename(link)}`,
          `path: ${link instanceof import_obsidian28.TFile ? link.path : link}`,
          "",
          body,
          D1
        ].join("\n")
  ...
  async cmd_probe_active_llm_elements() {
    const llm = await this.get_last_active_llm();
    if (!llm) {
      new import_obsidian28.Notice("No active LLM webview found");
      return;
    }
    const result = await llm.probe_action_elements();
    if (!result) {
      new import_obsidian28.Notice(`${llm.name}: probe failed`);
      return;
    }
    const okCount = [result.input, result.send, result.copy].filter(Boolean).length;
    new import_obsidian28.Notice(`${llm.name}: probe ${okCount}/3 (see console)`);
  }
  async cmd_copy_active_llm_profile_snippet() {
    ...
    try {
      ...
      new import_obsidian28.Notice(`${llm.name}: profile snippet copied`);
    } catch (e) {
      new import_obsidian28.Notice(`${llm.name}: copy failed, snippet in console`);
    }
  }
...
// main.ts
var NoteChainPlugin = class extends import_obsidian29.Plugin {
  constructor() {
    super(...arguments);
  ...
  async _onload_() {
    ...
    if (!this.easyapi.isMobile && this.settings.notechain.httpServerEnabled) {
      this.httpServer.start().then(() => {
        ...
        if (this.debug) {
          new import_obsidian29.Notice(`Failed to start HTTP Server: ${error.message}`, 5e3);
        }
      });
    }
    addCommands(this);
